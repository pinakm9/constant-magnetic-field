...
class LSTMForgetNet(tf.keras.models.Model):
    ...
    """
    def __init__(self, num_nodes, num_layers, out_dim, dtype=tf.float32, name = 'LSTMForgetNet', last_bias=True):
        super().__init__(dtype=dtype, name=name)
        self.num_nodes = num_nodes
        self.num_layers = num_layers
        self.out_dim = out_dim
        self.lstm_layers = [LSTMForgetBlock(num_nodes, dtype=dtype) for _ in range(num_layers)]
        self.final_dense = tf.keras.layers.Dense(units=out_dim, activation=None, dtype=dtype, name='endgame', use_bias=last_bias)
