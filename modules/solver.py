...
class Solver:
    def __init__(self, domain, value):
        self.domain = domain 
        self.value = value 
        self.A = arch.LSTMForgetNet(num_nodes=50, num_layers=2, out_dim=3, name="magnetic_potential", last_bias=False)
    ...
    def B(self, x, y, z):
        with tf.GradientTape(persistent=True) as tape:
            tape.watch([x, y, z])
            Ax, Ay, Az = tf.split(self.A(x, y, z), 3, axis=-1)
        ...
        return (Az_y - Ay_z), (Ax_z - Az_x), (Ay_x - Ax_y) 
    ...
    def train_step(self, optimizer, domain_data, boundary_data):
        with tf.GradientTape(persistent=True) as tape:
            loss = self.loss_energy(domain_data) + 1000.0 * self.total_loss_b(boundary_data) 
